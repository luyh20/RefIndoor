...
class Refindoor(ReferSegDataset):
    def __init__(self, data_root, split, img_size, max_img_size, img_type, dataset="refindoor",
                 max_query_len=30, bert_model='bert-base-uncased'):
        super(Refindoor, self).__init__(
            data_root=data_root,
            split=split,
            dataset=dataset,
            max_query_len=max_query_len,
    ...
    def __getitem__(self, idx):
        input_sample, target = super(Refindoor, self).__getitem__(idx)
        target = {k: torch.as_tensor(v) for k, v in target.items()}
        # target['boxes'] = torch.as_tensor(target['boxes'])
        img = Image.fromarray(input_sample["img"])
        if self._transforms is not None:
