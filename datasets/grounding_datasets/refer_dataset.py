...
class ReferSegDataset(data.Dataset):
    def __init__(self, data_root, split='train', dataset='refindoor',
                  max_query_len=30, bert_model='bert-base-uncased', img_type='RGB'):
        self.data_root = data_root
        self.dataset = dataset
        ...
        if split=='train':
            data_split_pth = osp.join(data_root, 'train', '{}_{}.json'.format(dataset, split))
        else:
            data_split_pth = osp.join(data_root, split, '{}_{}.json'.format(dataset, split))

        self.data = json.load(open(data_split_pth, 'r', encoding="utf-8"))

    def pull_item(self, idx):
        data_info = self.data[idx]
        text = data_info['text']
        bbox = data_info['bbox']
        rgb_img_path = data_info['rgb_path'].replace('\\', '/')
        depth_path = data_info['depth_path'].replace('\\', '/')
        mask_path = data_info['mask_path'].replace('\\', '/')
        ## box format: x1y1x2y2
        bbox = np.array(bbox, dtype=int)
        img_file = osp.join(parent_path, '../../data', rgb_img_path)
        depth = cv2.imread(osp.join(parent_path, '../../data', depth_path))
        img = cv2.imread(osp.join(parent_path, '../../data', rgb_img_path))
        mask = cv2.imread(osp.join(parent_path, '../../data', mask_path))
        ...
        if self.img_type=='RGB':
            ...
            print('the image type is error')
            return

        return img, mask, text, bbox, img_file
