...
class RefTR(nn.Module):
    def __init__(self, img_backbone, lang_backbone, vl_transformer,
        ...
        if num_feature_levels > 1:
            num_backbone_outs = len(self.img_backbone.strides)-2
            input_proj_list = []
    ...
    def forward(self, samples):
        ...
        if 'phrase' in samples.keys():
            ...
            # phrase_pooled_feat = sentence_feat
            # query_mask = torch.zeros((bsz, 1), device=sentence_mask.device).to(torch.bool)
            # mask_context = torch.zeros((query_mask.shape[0], query_mask.shape[1], query_mask.shape[1]), device=sentence_mask.device).to(torch.bool)
