...
from datasets import build_refer_dataset, build_refindoor
...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset', default='refindoor')
    ...
    parser.add_argument('--img_type', default='RGBD')
    ...
    parser.add_argument('--output_dir', default='outputs/refindoor3',
...
def main(args):
    ...
    dataset_train = build_refindoor('train', args)
    dataset_val = build_refindoor(args.test_split, args)
    ...
    if args.resume == '' and args.auto_resume:
        default_ckpt = output_dir / 'checkpoint_best.pth'
    ...
    if args.eval or args.resume:
        #for i, data_loader_val in enumerate(data_loader_val):
        test_stats, result = evaluate(
            model, criterion, postprocessors, data_loader_val, device, output_dir, visualize=args.eval
        )
        print(args.test_split, test_stats)
        # TODO fix bug here
        with (output_dir / f"{args.dataset}_{args.test_split}_result.json").open("w") as f:
    ...
    for epoch in range(args.start_epoch, stop_epoch):
        ...
        log_stats = {
                    **log_stats,
                    **{f'{args.test_split}_{k}': v for k, v in test_stats.items()},
                    }
